...
export default function LiveScreen() {
  ...
  const paramKey = String(Array.isArray(rawId) ? rawId[0] : rawId || "")
    .toUpperCase()
    .trim();

  const [selectedId, setSelectedId] = useState<string>(
    PATIENTS[paramKey] ? paramKey : "PT1",
  );
    ...
    <ScrollView style={{ flex: 1, backgroundColor: "#f8fafc", padding: 16 }}>
      ...
      <View style={S.card}>
        ...
        >
          ...
          <Text
            style={{ color: patient.color, fontWeight: "bold", fontSize: 12 }}
          >
      ...
      <View style={{ flexDirection: "row", gap: 8, marginBottom: 12 }}>
        ...
        <VitalBox
          ...
          value={`${patient.spo2}%`}
          unit="O2 Sat"
          color={patient.spo2 < 90 ? "#dc2626" : "#2563eb"}
        />
        <VitalBox
          label="BP"
          value={patient.bp}
          unit="mmHg"
          color={activeId === "PT4" ? "#dc2626" : "#16a34a"}
        />
      </View>

      {/* ECG Image */}
      <View style={S.card}>
        <Text style={{ color: "#0284c7", fontWeight: "bold", marginBottom: 8 }}>
          ⚡ LEAD II ECG WAVEFORM ({activeId})
        </Text>
        <View style={S.imgBox}>
          {activeImage ? (
            <Image
      ...
      <View style={S.card}>
        ...
        {result ? (
          <View
            style={[
              S.resBox,
              { borderColor: result.border, backgroundColor: result.bg },
            ]}
          >
            <Text
              style={{ color: result.color, fontWeight: "bold", fontSize: 15 }}
            >
          ...
          <Text style={{ color: "#64748b", fontSize: 12, marginBottom: 12 }}>
            กดปุ่มด้านล่างเพื่อเริ่มประมวลผลสัญญาณ ECG ด้วย AI
          </Text>
        )}
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          router.push({ pathname: "/history", params: { patientId: activeId } })
        }
      >
...
const VitalBox = ({ label, value, unit, color }: any) => (
  <View
    style={[
      S.card,
      { flex: 1, alignItems: "center", padding: 8, marginBottom: 0 },
    ]}
  >
    ...
    <Text
      style={{ color, fontSize: 18, fontWeight: "bold", marginVertical: 2 }}
    >
      {value}
    </Text>
